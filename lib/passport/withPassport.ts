...
import { Profile as ProfileBase } from 'passport-github';
...
// production-quality application, this would t——ypically be as simple as
// supplying the user ID when serializing, and querying the user record by ID
// from the database when deserializing.  However, due to the fact that this
// example does not have a database, the complete Github profile is serialized
// and deserialized.
passport.serializeUser((ghUser: Profile, done) => {
  const user = {
    username: ghUser._json.login,
    email: ghUser._json.email,
    tbn: ghUser._json.avatar_url,
    bio: ghUser._json.bio,
  };
  done(null, user);
});
...
export default (fn) => (req, res) => {
  if (!res.redirect) {
    ...
    return (res.redirect = (location: string) => {
